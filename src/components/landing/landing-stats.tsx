...
export function LandingStats() {
  return (
    ...
    >
      <div className="grid grid-cols-2 lg:grid-cols-4 border-2 border-border-neon bg-bg-2">
        {STATS.map((s, i) => (
          <div
            key={s.label}
            className={`p-8 sm:p-9 flex flex-col gap-3 ${
              i < STATS.length - 1 ? "lg:border-r-2" : ""
            } ${i % 2 === 0 ? "border-r-2 lg:border-r-2" : ""} ${
              i < 2 ? "border-b-2 lg:border-b-0" : ""
            } border-border-neon`}
          >
            <span
              className={`font-grotesk font-bold text-[40px] sm:text-[56px] md:text-[64px] leading-none tracking-[-0.03em] ${s.glow}`}
            >
              {s.value}
            </span>
